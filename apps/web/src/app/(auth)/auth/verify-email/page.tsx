...
export default function VerifyEmail() {
  ...
  useEffect(() => {
    sendCodeMutateAsync({ forcesend: false, type: "email" });
  }, []);
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    toast.promise(verifyCodeMutateAsync({ otp: values.otp, type: "email" }), {
      loading: "Verifying OTP...",
      success: "Email verified successfully!",
      error: (e: Error) => e.message || "Failed to verify email.",
    });
  }
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="flex gap-4 justify-between">
                <Button
                  onClick={(e) => {
                    e.preventDefault();
                    sendCodeMutateAsync({ forcesend: true, type: "email" });
                  }}
                >
                  Resend Code
                </Button>
